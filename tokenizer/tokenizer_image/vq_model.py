...
class VQModel(nn.Module):
    def __init__(self, config: ModelArgs):
        ...
        self.quant_conv = nn.Conv3d(config.z_channels, config.codebook_embed_dim, 1)
        self.post_quant_conv = nn.Conv3d(config.codebook_embed_dim, config.z_channels, 1)
...
class Encoder(nn.Module):
    def __init__(self, in_channels=1, ch=128, ch_mult=(1,1,2,2,4), num_res_blocks=2, 
...
class Decoder(nn.Module):
    def __init__(self, z_channels=256, ch=128, ch_mult=(1,1,2,2,4), num_res_blocks=2, norm_type="group",
                 dropout=0.0, resamp_with_conv=True, out_channels=1):
        super().__init__()
        self.num_resolutions = len(ch_mult)
        self.num_res_blocks = num_res_blocks

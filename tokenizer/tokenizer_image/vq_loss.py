...
class VQLoss(nn.Module):
    def __init__(self, disc_start, disc_loss="hinge", disc_dim=64, disc_type='patchgan', image_size=128,
                 disc_num_layers=3, disc_in_channels=1, disc_weight=1.0, disc_adaptive_weight = False,
    ...
    def forward(self, codebook_loss, inputs, reconstructions, optimizer_idx, global_step, last_layer=None, 
        ...
        if optimizer_idx == 0:
            ...
            # perceptual loss
            s0,s1,s2,s3,s4 = inputs.shape
            x_input = inputs.permute(0,4,1,2,3).reshape(-1,s1,s2,s3)
            x_hat2 = reconstructions.permute(0,4,1,2,3).reshape(-1,s1,s2,s3)
            inps = torch.cat((x_input,x_input,x_input), dim=1)
            recon = torch.cat((x_hat2,x_hat2,x_hat2), dim=1)

            p_loss = self.perceptual_loss(inps.contiguous(), recon.contiguous())
